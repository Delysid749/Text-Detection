...
def detect_identify_text(image, img_path):
    ...
    if image is None:
        ...
        if result is None or not result or len(result) == 0 or result[0] is None:
            ...
            print(f"PaddleOCR检测到的文本已保存到 {output_text_path}")

            # 打印检测到的文本和置信度
            for idx, text in enumerate(texts):
                print(f"PaddleOCR:text {idx + 1}: {text} (置信度: {scores[idx]:.2f})")

    # 记录总的执行时间
    overall_end_time = time.time()
    print(f"PaddleOCR文本检测、识别总执行时间: {overall_end_time - overall_start_time:.2f} 秒")
